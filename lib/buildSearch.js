...
module.exports = function(outFile, cb) {
  ...
  for (var i in tree) {
    ...
    var relLink = this.searchOptions.relPath ? determineRelLink(link, this.searchOptions.relPath) : link;
    ...
    for (var a in this.adapters) {
      if (this.adapters[a].search && item[a]) {
        results = results.concat(this.adapters[a].search(item[a], fullLink, item.title, section, this.options));
      }
    }
  }
...
function determineRelLink(link, relPath) {
    var relLink = link;
    if(_.isArray(relPath)){
        var matchedPath = _.find(relPath, function(path){
            return linkContainsRelPath(link, path);
        })
        if (matchedPath) {
            relLink = link.substring(link.indexOf(matchedPath) + matchedPath.length)
        }
    } else if (linkContainsRelPath(link, relPath)) {
        relLink = link.substring(link.indexOf(relPath) + relPath.length)
    }
    return relLink;
}

function linkContainsRelPath(link, relPath) {
    return link.indexOf(relPath) > -1;
}
